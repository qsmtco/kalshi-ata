...
class Position:
    ...
    # H5: Track fees for accurate P&L
    fees: float = 0.0
    ...
    def unrealized_pnl(self) -> float:
        """P&L if closed at current price (positive = profit), minus fees."""
        return self.market_value - self.cost_basis - self.fees
...
class PositionTracker:
    ...
    def __init__(self):
        ...
        self._lock = threading.Lock()  # Protect all position operations
    ...
    def get_position(self, ticker: str) -> Optional[Position]:
        ...
        with self._lock:
            ...
            return pos  # caller should not modify
    ...
    def reduce_position(self, ticker: str, count: int, reason: str = "", fee: float = 0.0) -> None:
        """Partially close a position. Fee is added to position's tracked fees."""
        with self._lock:
            if ticker not in self._positions:
                logger.warning(f"Tried to reduce non-existent position: {ticker}")
                return
            pos = self._positions[ticker]
            # Add fee to position's fee tracking
            if fee > 0:
                pos.fees += fee
            if count >= pos.count:
                self._positions.pop(ticker)
                logger.info(f"Position closed (reduce): {ticker} — reason: {reason}")
    ...
    def close_position(self, ticker: str, reason: str = "", fee: float = 0.0) -> None:
        """Fully close a position. Fee is subtracted from realized P&L."""
        with self._lock:
            if ticker in self._positions:
                pos = self._positions.pop(ticker)
                # Add fee to position for accurate P&L calculation
                pos.fees += fee
                logger.info(
    ...
    def sync_from_api(self, api_positions: list) -> int:
        ...
        Uses MERGE strategy: only update/add from API, don't remove local-only
        positions (they may be pending orders).

        Handles two formats:
          - market_positions: has 'ticker' + 'position_fp' (fixed-point count)
          - event_positions: has 'event_ticker' + 'total_cost_shares_fp'

        Returns number of positions restored/updated.
        """
        with self._lock:
            restored = 0
            for p in api_positions:
                ...
                # MERGE: update existing or add new, but never remove local-only positions
                if ticker in self._positions:
                    # Update existing position from API data
                    pos = self._positions[ticker]
                    pos.count = count
                    pos.avg_fill_price = avg_price
                    pos.last_updated = datetime.now(timezone.utc)
                    logger.info(f"Position synced (updated): {ticker} -> {count} @ ${avg_price:.4f}")
                else:
                    self._positions[ticker] = Position(
                        ticker=ticker,
                    ...
                    logger.info(f"Position synced (new): {ticker} — {count} @ ${avg_price:.4f}")
                restored += 1
            logger.info(f"PositionTracker synced from API: {restored} positions merged")
            return restored
